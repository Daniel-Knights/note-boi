 ...
 * Specifically intended for `n.editNote`, `n.deleteNotes`, and `n.importNotes`.
 */
export async function waitForAutoSync(
  ...
  await waitUntil(() => {
    return (
      calls.invoke.has('edit_note') ||
      calls.invoke.has('delete_notes') ||
      calls.invoke.has('import_notes')
    );
  });
